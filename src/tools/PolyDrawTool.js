...
class PolyDrawTool extends Tool {
  ...
  onPress(action) {
    ...
    if (this.model.controlsactive && !this.model.activityInProgress) {
      ...
      this.model.controlsactive &&
      this.model.activityInProgress === true
    ) {
      if (
        !(
          this.model.clicks >= 2 && // each click registers twice
          this.model.clicks % 2 === 0
        )
      ) {
        // only consider one click
        return;
      }

      const lastAnnotation =
        this.model.annotations[this.model.annotations.length - 1];
      if (!(lastAnnotation && lastAnnotation[0] === "path")) {
        return;
      }

      const distanceToStart = pointDistance(lastAnnotation[1].points[0], {
        x: x,
        y: y,
      });

      if (
        distanceToStart < this.THRESHOLD &&
        lastAnnotation[1].points.length > 1
      ) {
  ...
  onMove(action) {
    if (this.model.activityInProgress === true) {
      ...
      if (
        ...
        d3.select("#poly_overlay").remove();
        createCircleOverlay(
          lastAnnotation[1].points[0].x,
          lastAnnotation[1].points[0].y,
          this.THRESHOLD
        );
      } else {
        d3.select("#poly_overlay").remove();
